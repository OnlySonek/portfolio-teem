...
document
  .querySelectorAll(
    "a, button, .side-dot, .tool-tag, .tool-tag-cyan, .social-btn, .social-btn-cyan",
  )
  .forEach((el) => {
...
const purplePeak = {
  ...
  color: "#fff",
  y: -5,
  boxShadow: "0 10px 28px rgba(108,99,255,0.45)",
};
const cyanBase = {
...
const cyanPeak = {
  ...
  color: "#fff",
  y: -5,
  boxShadow: "0 10px 28px rgba(0,212,255,0.45)",
};

const socialPurpleBase = {
...
const socialPurplePeak = {
  backgroundColor: "rgba(108,99,255,0.2)",
  borderColor: "rgba(108,99,255,0.8)",
  color: "#fff",
  y: -7,
  boxShadow: "0 12px 30px rgba(108,99,255,0.5)",
};
const socialCyanBase = {
...
function fireGlowM1() {
  ...
  glowSequence("#m1Links .social-btn", {
    base: socialPurpleBase,
    peak: socialPurplePeak,
  });
}
function fireGlowM2() {
  glowSequence("#m2Tools .tool-auto-cyan", { base: cyanBase, peak: cyanPeak });
  glowSequence("#m2Links .social-btn-cyan", {
    base: socialCyanBase,
    peak: socialCyanPeak,
  });
}
...
function spawnConstellation() {
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    circle.setAttribute(
      "fill",
      isPurple ? "rgba(108,99,255,0.7)" : "rgba(0,212,255,0.5)",
    );
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      for (let j = i + 1; j < COUNT; j++) {
        ...
        if (dist < CONNECT_DIST) {
          ...
          l.setAttribute("x1", d.x);
          l.setAttribute("y1", d.y);
          l.setAttribute("x2", dots[j].x);
          l.setAttribute("y2", dots[j].y);
...
function animateIn(index) {
  ...
  if (index === 1) {
    ...
    gsap.fromTo(
      "#aboutLineTop",
      { width: "0%" },
      { width: "60%", duration: 1.4, ease: "expo", delay: 0.1 },
    );
    gsap.fromTo(
      "#aboutLineBot",
      { width: "0%" },
      { width: "40%", duration: 1.4, ease: "expo", delay: 0.3 },
    );
    ...
    // stat cards stagger
    gsap.fromTo(
      ["#aboutStat1", "#aboutStat2", "#aboutStat3"],
      { y: 30, opacity: 0 },
      {
        y: 0,
        opacity: 1,
  ...
  if (index === 2) {
    ...
    gsap.fromTo(
      "#m1Info",
      {
        x: window.innerWidth < 769 ? 0 : 60,
        y: window.innerWidth < 769 ? 30 : 0,
        opacity: 0,
      },
  ...
  if (index === 4) {
    ...
    gsap.fromTo(
      "#tName1",
      { y: "100%" },
      { y: "0%", duration: 0.8, ease: "expo", delay: 0.75 },
    );
    gsap.fromTo(
      "#tName2",
      { y: "100%" },
      { y: "0%", duration: 0.8, ease: "expo", delay: 0.8 },
    );
    // roles fade
    gsap.fromTo(
      ["#tRole1", "#tRole2"],
    ...
    gsap.fromTo(
      "#teamFooter",
      { opacity: 0, y: 12 },
      { opacity: 1, y: 0, duration: 0.7, delay: 1.4 },
    );
  }
  ...
  if (index === 5) {
    ...
    gsap.fromTo(
      "#s5 p",
      { y: 20, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.7, ease: "expo", delay: 0.1 },
    );
    // title
    gsap.fromTo(
      "#s5 h2",
      { y: "80%", opacity: 0, scale: 1.1 },
      { y: "0%", opacity: 1, scale: 1, duration: 1, ease: "expo", delay: 0.2 },
    );
    // project cards
    gsap.fromTo(
      ".project-card",
...
function resetSection(i) {
  ...
  if (i === 0) {
    ...
    if (fw) fw.innerHTML = "";
    if (constellationAnim) {
      cancelAnimationFrame(constellationAnim);
  ...
  if (i === 1) {
    ...
    gsap.set(["#aboutStat1", "#aboutStat2", "#aboutStat3"], {
      y: 30,
      opacity: 0,
    });
  ...
  if (i === 2) {
    ...
    if (n1) {
      n1.textContent = "Abdelrhman";
      gsap.set(n1, { opacity: 1 });
    }
    if (l1) {
      l1.textContent = "Sherif";
  ...
  if (i === 3) {
    ...
    if (n2) {
      n2.textContent = "Seif";
      gsap.set(n2, { opacity: 1 });
    }
    if (l2) {
      l2.textContent = "El-sayed";
  ...
  if (i === 4) {
    ...
    gsap.set("#teamLabelMobile", { y: 16, opacity: 0 });
    ...
    document
      ...
      .forEach((b) => gsap.set(b, { width: "0%" }));
  }
  if (i === 5) {
    gsap.set("#s5 p", { y: 20, opacity: 0 });
    gsap.set("#s5 h2", { y: "80%", opacity: 0, scale: 1.1 });
...
function goTo(index) {
  if (busy || index === current || index < 0 || index >= sections.length)
    return;
  busy = true;
  playTransition(() => {
    resetSection(index);
